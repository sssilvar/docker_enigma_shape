...
if($#ARGV < 5)
{
	die("usage: perl label_to_mesh.pl [bin_dir] [working_dir] [freesurfer bin] [aseg_file].aseg [label_id](int) [mesh_output].m");
}
...
$Cmd_str = "$freesurfer_bin/mri_convert $aseg_file ${ana_name}.img";
print "$Cmd_str\n";
system $Cmd_str;


$Cmd_str = "$bin_dir/MeshfromAnalyze -p $smoothing_weights -m $smoothing_weights -d 1 -e -b -1 40 -M $label_id ${ana_name}.img ${ana_name}_MC.m ${ana_name}_LS.img";
print "$Cmd_str\n";
system $Cmd_str;
