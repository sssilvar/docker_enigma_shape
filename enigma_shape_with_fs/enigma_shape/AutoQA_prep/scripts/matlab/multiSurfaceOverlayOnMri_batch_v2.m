function multiSurfaceOverlayOnMri_batch_v2(surfNames, subjlist, imgNames, ...
    ...
    try
        ...
        switch overlayFlag
            case 0 % all-in-one-figure, common bounding box.
                fHandles = multiSurfaceOverlayDetailedCheck_v2(img, vertices, faces, surfColor, subjlist{it}, surfNames, 0, annotFlag, scaleIntensitiesFlag, IsDisplaySliceNumber);
            case 1 % compare - separate figures, common bounding box- on same slices.
                % Extents in each figure decided by all the surfaces
                fHandles = multiSurfaceOverlayDetailedCheck_v2(img, vertices, faces, surfColor, subjlist{it}, surfNames, 1, annotFlag, scaleIntensitiesFlag, IsDisplaySliceNumber);
            case 2 % separate - separate figures, separate bounding box
                % Extents in each figure decided by respective surfaces
                fHandles=gobjects(size(surfNames));
                for ss = ssList
                    fHandles(ss) = multiSurfaceOverlayDetailedCheck_v2(img, vertices(ss), faces(ss), ...
