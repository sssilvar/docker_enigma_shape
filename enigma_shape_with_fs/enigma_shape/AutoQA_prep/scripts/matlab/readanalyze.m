...
if fid<0
    error('mial:dataIO:FailOpenForRead', 'Could not open the file %s for reading', [filename,'.hdr']);
end
fseek(fid,40,'bof');
...
if fid<0
    error('mial:dataIO:FailOpenForRead', 'Could not open the file %s for reading', [filename,'.img']);
end
